...
from app.database.mongo_store import get_history, get_startup

router = APIRouter(tags=["History"])

@router.get("/history")
def history(x_user_id: int = Header(...)) -> list[dict]:
    """List saved startup packages, newest first."""
    return get_history(x_user_id)

@router.get("/history/{startup_id}")
def startup_history_item(startup_id: int, x_user_id: int = Header(...)) -> dict:
    """Fetch a complete saved startup package by identifier."""
    startup = get_startup(startup_id, x_user_id)
    if startup is None:
...
def startup_report(startup_id: int, x_user_id: int = Header(...)) -> dict:
